# A.G.1 implementation evidence — 2026-09-08
...
- User-story documentation moves X1 to A.G.1 without renumbering other appendix entries.
...
- Isolated broader core suite: 275/276 passed before the two added recipient tests. The sole failure is the existing transfer-recovery-report test expecting the text `Do not resubmit`; A.G.1 does not change that report implementation.
...
UI revision: A.G.1. Game Wallet now has Import initially, Details/Logout after selection. Browser verified public-only console output and persistent deselection after Logout/reload using synthetic storage. Four controller tests pass including retention after logout.
