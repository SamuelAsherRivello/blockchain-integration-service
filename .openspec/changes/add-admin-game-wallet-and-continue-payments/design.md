...
Deliver wallet setup first, then configured payments. Keep existing player records untouched and read legacy continuation records additively. Publish the configured demo and update the named game consumer. If verification fails, disable new Continue payments in the host while retaining account access and reconciliation; do not revert to sink fallback or erase journals. Documentation moves the former X1 story to A.G.1 under F. Game Wallet and updates its current links. Leave historical change directory names intact. Do not renumber the other appendix stories as part of A.G.1; the deferred trophy X2 naming collision stays recorded in its proposal.
