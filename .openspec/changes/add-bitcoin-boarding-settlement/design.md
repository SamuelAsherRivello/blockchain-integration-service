...
- Quote using SDK Ramps with a capture-only settle facade: onboard selected confirmed unexpired Bitcoin inputs, or offboard selected spendable VTXOs. Exclude recoverable/unrolled/locked funds. For asset-bearing inputs, reserve minimum owned Arkade change; the installed SDK assigns all assets to that change during settlement. Bind exact asset inventories in the quote fingerprint, validate the asset intent before registration, and persist and reconcile the expected asset-change script, sats and quantities. `maxSats` is the withdrawable amount; `inputSats` records the full input value for change reconciliation, with legacy records falling back to `maxSats`. A reverse destination is derived by this wallet's getBoardingAddress, never supplied externally. This matches the Bitcoin balance's boarding definition and existing script/exit conditions. Read-only SDK output construction validates the address format; actual confirmed receipt remains a live gate.
