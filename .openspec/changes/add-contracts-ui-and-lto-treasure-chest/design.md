...
Each operation still validates same-origin signer identities, Signet/operator compatibility, zero fees, eligible unreserved inputs with every asset retained in game-owned change, minimum output/change requirements, session identity and the immutable deadline. Unknown submissions retain recovery and exclusivity. Unsupported conditions reject the operation truthfully; no ordinary-send fallback or synthetic confirmation is added. The client enforces 90-second claim eligibility; it is not an on-chain upper-bound timelock.
...
Browser timers are hints: recompute eligibility from the persisted deadline on query, click and visibility return. Do not pause the deadline with the game or assume a suspended tab executes cleanup. Timer-based cleanup resumes when the client runs. A changed fee schedule prevents new zero-fee offers; unresolved contracts remain visible and require an explicit supported recovery decision rather than silent additional spending.
