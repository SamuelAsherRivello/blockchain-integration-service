...
Alternative considered: create a second A.G.2-only storage record and synchronize it with Admin. Rejected because it would introduce merge/ordering ambiguity, fail the required A.G.1/A.G.2 shared selection, and risk stale game state.

### 2. A.G.2 is a narrow sibling of the player onboarding flow
...
Alternative considered: have A.G.1 write a file or static package config for consuming games. Rejected because it cannot give a deployed browser runtime mutation access and would violate the required per-origin user-facing A.G.2 setup model.
...
- [A selected game wallet cannot be shared automatically with another origin] → A.G.2 is present in every user-facing BIS host and explicitly owns standalone-game setup; same-origin Admin/Preview sharing remains automatic.
...
- [A wallet change leaves a real old contract on Arkade] → retain it only for bounded reconciliation while hiding it from the replacement wallet's D.P.2 presentation and blocking cross-wallet actions.
...
- [A.G.2 reuses recovery behavior] → reuse the established private UI/storage boundaries, add tests that public context events, logs, and Admin console values contain no recovery material.
