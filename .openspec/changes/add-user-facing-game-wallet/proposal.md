...
- Define the canonical catalog: **A.G.1. Game Wallet (Admin-facing)**, **A.G.2. Game Wallet (User-facing)**, and **B.G.2. Board Game Wallet**.
- Make A.G.1 and A.G.2 two entry points to the same browser-and-origin-scoped game-wallet selection. B.G.2 remains an Admin-only action that uses that same selected wallet.
...
- Remove the obsolete `BIS/packages/wallet-service` package and its hosted-wallet adapters, scripts, service-only tests, and documentation after all A.G.1/A.G.2/B.G.2 and D.P.2 callers use the local controller.
...
- `admin-game-wallet`: Renumber Admin controls as A.G.1/B.G.2 and bind them to the same local game-wallet selection as A.G.2.
