...
- [x] 1.1 Extend the local game-wallet controller with the non-secret create/select lifecycle and selection-change signal needed by A.G.1/A.G.2 consumers, preserving encrypted same-origin persistence; verify focused controller tests cover reload, cross-context selection, invalid setup, logout, and no player-wallet mutation.
...
- [x] 1.3 Ensure local LTO storage/reconciliation is scoped to the selected game-wallet identity and rejects stale signer actions after selection changes; verify focused LTO tests cover same-origin exclusivity, reload reconciliation, and clean D.P.2 epoch boundaries.

## 2. A.G.2 user-facing game-wallet UI
...
- [x] 4.1 Remove hosted LTO selection from the D.P.2 composition path and use the local direct Arkade contract adapter for funding, claim, refund, persistence, and reconciliation; verify focused LTO tests exercise those operations with no BIS application server.
...
- [x] 4.4 Update treasure-session integration to capture a game-wallet selection epoch at Start, retain signer stability for an active run, and start the next run with no old wallet D.P.2 history; verify session tests for selection, replacement, logout, and unresolved prior work.
...
- [x] 4.6 Make the D.P.2 presentation reset its 90-second countdown only when the fresh replacement session is established; verify failed cleanup does not show a false fresh countdown and successful replacement visibly resets the timer.
