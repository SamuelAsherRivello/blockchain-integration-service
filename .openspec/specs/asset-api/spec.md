...
BIS SHALL bind each operation ID to its complete request and account. Repeating that operation SHALL NOT issue another asset. Reusing an ID with different inputs SHALL be rejected. Names and tickers SHALL NOT define uniqueness: separate intentional operations after completion may mint distinct assets with identical metadata. An unresolved mint SHALL block new mint operations for that account until reconciled; a missing asset alone SHALL NOT authorize resubmission. Missing coordination or durable storage SHALL prevent submission.
...
- **WHEN** a previous operation completed and the caller explicitly starts a new operation with identical metadata
...
- **AND** an unresolved result returns outcome-unknown rather than permitting a new mint
