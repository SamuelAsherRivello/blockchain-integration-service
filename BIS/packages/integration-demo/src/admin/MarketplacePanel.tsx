...
export function MarketplacePanel({ controller, onLog }: {controller?: ReturnType<typeof createBisGameWallet>; onLog(value: unknown): void}) {
  ...
  async function mintCatalog() {
    ...
    try {
      ...
      const batch = await mintAndVerifyMarketplaceCatalog({mint:target.mint,listAssets:()=>controller.listAssets()}, target.isCurrent, progress => {
        ...
        onLog({operation:'H1. Marketplace mint',status:'pending',message,progress});
      });
  ...
  async function burnCatalog() {
    ...
    try {
      ...
      const batch=await burnAllMarketplaceItems(controller,()=>controller.getState().profileId===targetProfile, progress => {
        ...
        onLog({operation:'H2. Burn All Items for Marketplace',status:'pending',message,progress});
      });
  ...
  async function listAllItems() {
    if (!controller || busy) return; setBusy(true);
    const operation = 'H3. List All Items For Marketplace';
  ...
  return <StorySection title="H. Marketplace" className="marketplace-panel"><p className="story-summary">Mint or burn the nine-item Stealth &amp; Steel catalog in the active game wallet.</p><StoryButton label="H1. Mint Items for Marketplace"><button disabled={!active || busy} onClick={() => void mintCatalog()}>Mint catalog</button></StoryButton><StoryButton label="H2. Burn All Items for Marketplace"><button disabled={!active || busy} onClick={() => void burnCatalog()}>Burn all marketplace items</button></StoryButton><StoryButton label="H3. List All Items For Marketplace"><button disabled={!active || busy} onClick={() => void listAllItems()}>List all marketplace items</button></StoryButton></StorySection>;
}
