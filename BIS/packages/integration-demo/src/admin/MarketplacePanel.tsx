...
export function MarketplacePanel({ controller, onLog, children }: {controller?: ReturnType<typeof createBisGameWallet>; onLog(value: unknown): void; children?: ReactNode}) {
  ...
  return <div className="marketplace-panel"><p className="story-summary">Mint or burn the nine-item Stealth &amp; Steel catalog in the active game wallet.</p><StoryButton label="C.G.3. Burn All Items for Marketplace"><button disabled={!active || busy} onClick={() => void burnCatalog()}>Burn all marketplace items</button></StoryButton><StoryButton label="C.G.4. List All Items For Marketplace"><button disabled={!active || busy} onClick={() => void listAllItems()}>List all marketplace items</button></StoryButton>{children}<StoryButton label="C.G.2. Mint Items for Marketplace"><button disabled={!active || busy} onClick={() => void mintCatalog()}>Mint catalog</button></StoryButton></div>;
}
