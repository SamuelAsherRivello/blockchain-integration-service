...
export type TreasureSnapshot={id:string;reference:string;expiresAt:number;status:TreasureStatus;offered:boolean;reason?:string;playerId?:string;gameId?:string;contractId?:string;gameVersion?:number};
export function createTreasureSession(options:{context:BisContext;offers:ReturnType<typeof createBisLto>;gameWallet:ReturnType<typeof createBisGameWallet>;now?:()=>number;newId?:()=>string}):{
  getState():{status:TreasureStatus;remainingSeconds:number;sessionId?:string;contractId?:string;reason?:string};
  subscribe(listener:()=>void):()=>void;start():Promise<BisContractActionResult|undefined>;end():void;inspect():Promise<void>;
