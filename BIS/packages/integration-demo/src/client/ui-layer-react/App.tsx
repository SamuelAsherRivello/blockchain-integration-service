...
import { createBisContinue, createBisGameWallet, createBisLto, testNetwork } from '@bis/integration';
import { TreasureLtoPanel } from '../admin-layer/TreasureLtoPanel';
import { StoryAction } from '../admin-layer/StoryAction';
import { createBisContext, createBisAdminContext, createBisUi, type BisState, type BisContext } from '@bis/integration';
import { GameWalletPanel } from '../admin-layer/GameWalletPanel';
import { MarketplacePanel } from '../admin-layer/MarketplacePanel';
import { MintAssetDialog } from '../admin-layer/MintAssetDialog';
import { prepareMintDestination, type MintDestination } from '../admin-layer/mint-destination';
import '@bis/integration/style.css';
import { AdminPanel } from '../admin-layer/AdminPanel';
import { selectAccountStory } from '../admin-layer/selectAccountStory';
import { GamePreview } from '../preview-layer/GamePreview';
import { CompletionPreview } from '../preview-layer/CompletionPreview';
...
export function App({ contextFactory = createBisContext, gameWalletFactory = createBisGameWallet }: { contextFactory?: typeof createBisContext; gameWalletFactory?: typeof createBisGameWallet } = {}) {
  ...
  async function fundAccount(explorer = false) {
    ...
    const network=current.context.getState().network;
    if (!network) { setConsoleLines(['Choose a test network before opening funding tools.']); return; }
    const selectedNetwork=testNetwork(network);
    const urls = explorer ? ['about:blank'] : selectedNetwork.faucetUrls;
    ...
    try {
      ...
      if (explorer) {
        if (tabs[0]) tabs[0].location.href = selectedNetwork.bitcoinExplorerAddressUrl(address);
        return;
      }
  ...
  const openRuntimeDeveloper = useCallback(() => {
    setCompletionOpen(false);
    setSelected('A.G.2');
    session.current?.ui.openDeveloperDialog();
  }, []);
  function start() {
    ...
    const ui = createBisUi(context, {gameWallet});
    ...
    return { context, adminContext, ui, gameWallet, stop() { continueController.current?.dispose(); continueController.current=undefined; unsubscribeEvents(); unsubscribe(); ui.unmount(); gameWallet?.dispose(); context.dispose(); } };
  }
  useEffect(() => {
    let cancelled = false;
  ...
  function openOnboarding() {
    const context = session.current?.context;
    if (!context?.openAccountOnboarding || context.getState().phase !== 'active') return;
    setCompletionOpen(false);
    setSelected('B.P.8');
    context.openAccountDialog();
    context.openAccountOnboarding();
  }
  ...
  return <div className="demo-app">
    ...
    <SplitWorkspace>
      <AdminPanel network={state?.network} onOpenOnboarding={openOnboarding} contracts={<><TreasureLtoPanel context={runtimeContext} offers={lto} gameWallet={gameWalletController} onLog={result=>setConsoleLines(previous=>[...previous.slice(-19),`D.G.1 / D.P.2. LTO Treasure Chest\n${JSON.stringify(result,null,2)}`])}/><StoryAction id="D.P.1" label="Contracts UI" disabled={!state?.profileId} onClick={()=>{setSelected('D.P.1');session.current?.context.openAccountDialog();session.current?.context.openAccountContracts?.();}}/></>} marketplace={<MarketplacePanel controller={gameWalletController} onLog={result => logAsset('C. Game Wallet Marketplace', result)}><StoryAction id="C.G.1" label={gameMintPresent || (!!state?.hasProfile && state.phase === 'active') ? "Mint Asset & Send" : "Mint Asset & Send (Awaiting Wallet)"} disabled={!(gameMintPresent || (!!state?.hasProfile && state.phase === 'active')) || assetBusy} onClick={()=>void openMint()} /></MarketplacePanel>} mintAvailable={gameMintPresent || (!!state?.hasProfile && state.phase === 'active')} mintReason="Awaiting Wallet" playerActive={!!state?.hasProfile && state.phase === 'active'} continueReason={continueReadiness.reason} continueAvailable={!!state?.hasProfile && state.phase === 'active' && continueReadiness.canPay} gameWallet={<GameWalletPanel controller={gameWalletController} mode="account" onRecipientChange={onRecipientChange} onOpenDeveloper={openRuntimeDeveloper} onDetails={details => logAsset('Game Wallet', details)} />} gameWalletBoard={<GameWalletPanel controller={gameWalletController} mode="board" onOpenDeveloper={openRuntimeDeveloper} onDetails={details => logAsset('Game Wallet', details)} />} onCompleteLevel={()=>{session.current?.context.closeAccount();setCompletionOpen(true);setSelected('C.P.1');}} completionOpen={completionOpen} onShowToast={showToast} onShowToastWithIcon={showToastWithIcon} canShowToast={!!state} onContinue={()=>void requestContinue()} continueBusy={continueBusy} onMint={()=>void openMint()} assetBusy={assetBusy} consoleOutput={consoleLines.join("\n\n")} selected={selected} accountOpen={completionBusy || state?.view === 'account' || state?.phase === 'loading' || state?.phase === 'resetting'} canReset={!completionBusy && state?.phase !== 'loading' && state?.phase !== 'resetting' && (!!selected || !!state?.canReset)} onSelect={selectStory} onReset={()=>void reset()} canFund={!!state?.hasProfile && state.phase === 'active'} funding={funding} onFund={()=>void fundAccount()} onExplorer={()=>void fundAccount(true)} />
