...
} from '../../src/client/admin-layer/marketplace-catalog.ts';
...
test('Admin does not contain a development-only static Marketplace catalog publisher', async () => {
  const [panel, vite] = await Promise.all([
    readFile(new URL('../../src/client/admin-layer/MarketplacePanel.tsx', import.meta.url), 'utf8'),
  ...
  const catalogSource = await readFile(new URL('../../src/client/admin-layer/marketplace-catalog.ts', import.meta.url), 'utf8');
...
test('C.G.2, C.G.3, and C.G.4 report Marketplace progress and results only through the Admin Console', async () => {
  const panel = await readFile(new URL('../../src/client/admin-layer/MarketplacePanel.tsx', import.meta.url), 'utf8');
