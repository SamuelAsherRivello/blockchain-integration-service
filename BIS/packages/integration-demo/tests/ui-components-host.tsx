...
document.getElementById('run')!.onclick = async () => {
  ...
  try {
    ...
    for (const id of ['A.P.1', 'B.P.1', 'C.G.1']) story(id).click();
    check(actions.join('|') === 'A.P.1|B.P.1|C.G.1', 'Correct callbacks');
    check(!story('C.G.1').hasAttribute('aria-pressed') && !story('C.G.1').querySelector('.story-arrow'), 'Special actions retain presentation');
    root.render(admin(true)); await tick(); check(['A.P.1', 'B.P.1', 'C.G.1'].every(id => story(id).disabled), 'Account disables story actions');
