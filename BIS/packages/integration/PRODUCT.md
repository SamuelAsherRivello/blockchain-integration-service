...
Game developers embed the public integration API and its production React UI in browser games. Players create or restore an account on either the Signet or Mutinynet network, then use account, balance, asset, payment, and recovery flows inside the game experience.

## Product Purpose

Provide a reusable browser-game integration layer for blockchain-enabled account, payment, asset, and contract functionality on Signet and Mutinynet while keeping game code independent of wallet and Arkade SDK concerns.

## Positioning

The package exposes a minimal game-facing API while separating Core workflow/state, player-facing React UI, and Arkade SDK integration. It uses real selected-network behavior rather than simulated transaction outcomes.
...
- Signet and Mutinynet network options; Arkade SDK is the v1 integration.
...
- Make real selected-network state clear without making wallet setup mandatory for play.
