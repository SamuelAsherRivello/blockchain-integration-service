...
import { SeedWordsHeading, TestWalletWarning } from './RecoveryPhrasePanel';
import { phraseWords, RECOVERY_WORD_COUNT, validRecovery, wordValidity } from '../state-layer-core/recovery-validation';

export function RecoveryPhraseEntry({ editable, disabled, hideWhen = false, submitLabel, submitDisabled = false, onSubmit, onBack, backDisabled = false, message }: {
  editable: boolean;
  disabled: boolean;
  hideWhen?: boolean;
  submitLabel: string;
  ...
  useEffect(() => {
    if (!hideWhen) return;
    setShow(false); revision.current++; setPasting(false);
  }, [hideWhen]);
  ...
  async function paste() {
    if (!editable || disabled || pasting) return;
    ...
    try {
      ...
      if (alive.current && revision.current === current) setPasteError('Could not read the clipboard. Try again or type the words manually.');
    } finally {
      if (alive.current && revision.current === current) setPasting(false);
    }
  }
  return <>
    <TestWalletWarning />
    <SeedWordsHeading shown={show} disabled={disabled || pasting} onToggle={() => setShow(current => !current)} action={<PasteButton reading={pasting} disabled={!editable || disabled || pasting} onClick={() => void paste()} />}>
    ...
    <div className="bis-actions">
      <button className="bis-button bis-primary" disabled={disabled || submitDisabled || pasting || !valid} onClick={() => { revision.current++; setShow(false); onSubmit(words.join(' ')); }}>{submitLabel}</button>
      <button className="bis-button bis-back" disabled={backDisabled} onClick={onBack}>Back</button>
    </div>
  </>;
}
