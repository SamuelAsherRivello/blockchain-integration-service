import {eligibleUnreservedCoins, walletReservations} from '../state-layer-core/wallet-reservations.ts';
import { verifiedBoardingCommitment } from '../state-layer-core/boarding-reconciliation.ts';
import { hasLiveBoardingWait, liveBoardingState } from '../state-layer-core/live-boarding-wait.ts';
import { settlementTimeoutMs, settlementDiagnostic } from '../state-layer-core/boarding-status.ts';
...
import { boardingAssets, type BoardingAssetChange } from '../state-layer-core/boarding-assets.ts';
...
import type { TestNetwork } from '../state-layer-core/test-network.ts';
import { requireZeroFeePolicy, walletPolicyAvailability } from '../state-layer-core/wallet-network-policy.ts';
import { boardingAmounts, assertQuoteUnchanged, type BoardingQuote } from '../state-layer-core/boarding-quote.ts';
import { readBoardingRecord, readBoardingRecords, writeBoardingRecord, createBoardingAttempt, recoverPreparedBoarding, withWalletMutation, type BoardingRecord } from '../state-layer-core/boarding-record.ts';
import { withBrowserMutation } from '../state-layer-core/logout-cleanup.ts';
import { runBoardingWorker } from '../state-layer-core/boarding-execution.ts';
import { recordBoardingProgress, withBoardingRecordLock, type BoardingStage, type BoardingAction } from '../state-layer-core/boarding-record.ts';
...
async function plan(wallet:ReadonlyWallet,profileId:string,requested?:number,direction:BoardingQuote['direction']='to-arkade',network:TestNetwork='signet') {
  ...
  if(retained.length) {
    ...
    // arkd removes onchain receivers from its leaf but preserves asset vout
    // indices from the intent. Keep asset change at index 0 in BOTH. Ramps
    // returns Bitcoin first; reorder before the SDK constructs/signs the intent,
    // so its onchain_output_indexes and asset packet describe the same layout.
    params={...params,outputs:[params.outputs[1],params.outputs[0]]};
...
export function inspectBoardingAssets(proof:string,params:SettleParams,change:BoardingAssetChange) {
  ...
  if(params.outputs.length!==2||hex(ArkAddress.decode(params.outputs[0].address).pkScript)!==change.script)throw Error('Transfer asset change must precede Bitcoin output.');
  const recipients=params.outputs.map((o,i)=>({address:o.address,amount:Number(o.amount),...(i===0?{assets:change.assets.map(a=>({assetId:a.assetId,amount:BigInt(a.amount)}))}:{})}));
  ...
  const own=tx.getOutput(0),extension=tx.getOutput(2);
...
async function runBoarding(account:AccountSecret,quote:BoardingQuote,isCurrent:()=>boolean,onRegistered:(record:BoardingRecord)=>void,operationId:string):Promise<BoardingRecord> {
  ...
  const assertSigningActive=()=>{
    if(!active||settlementAbort.signal.aborted||!isCurrent()||Date.now()>=deadline)throw Error('Transfer signing session ended. Reconcile the original transfer.');
  };
  const eventStream=provider.getEventStream.bind(provider);
  provider.getEventStream=(signal,topics)=>eventStream(AbortSignal.any([signal,settlementAbort.signal]),topics);
  ...
  provider.submitTreeNonces=async(...args)=>{assertSigningActive();await observe('signing','tree-nonces');assertSigningActive();await nonces(...args);};
  const signatures=provider.submitTreeSignatures.bind(provider);
  provider.submitTreeSignatures=async(...args)=>{assertSigningActive();await observe('signing','tree-signatures');assertSigningActive();await signatures(...args);};
