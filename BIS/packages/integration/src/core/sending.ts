...
export type SendRecord = {version:1;id:string;profileId:string;status:'pending'|'succeeded';transactionId:string;quote:BisSendQuote;inputs:{txid:string;vout:number}[];recipientScript:string;change?:{script:string;sats:number;assets:{assetId:string;amount:string}[]}};
