...
const journalKey=(profileId:string,network:TestNetwork='signet')=>`bis-${network}-wallet-operations-v2:${encodeURIComponent(profileId)}`;
type Journal={version:2;profileId:string;operations:ReservedOperation[]};
function readJournal(profileId:string,network:TestNetwork='signet'):Journal|undefined {
  const raw=localStorage.getItem(journalKey(profileId,network));if(raw===null)return;
  try {
...
export function migrateWalletReservations(profileId:string,network:TestNetwork='signet') {
  readJournal(profileId,network);
  const operations=walletReservations(profileId,network);
  const raw=JSON.stringify({version:2,profileId,operations} satisfies Journal),key=journalKey(profileId,network);
  localStorage.setItem(key,raw);
  if(localStorage.getItem(key)!==raw)throw new ReservationError('Wallet reservations could not be saved.');
}
export function walletReservations(profileId:string, network:TestNetwork = 'signet'):ReservedOperation[] {
  ...
  for(const r of readAssetRecords(profileId,network))if(r.status==='pending')operations.push({id:`mint:${r.request.operationId}`,transactionId:r.transactionId});
  for(const burn of readBurnRecords(profileId,network))if(burn.status==='pending')operations.push({id:`burn:${burn.id}`,transactionId:burn.transactionId,inputs:burn.inputs});
  for(const delivery of readAssetDeliveryRecords(profileId,network))if(delivery.status==='pending')operations.push({id:`delivery:${delivery.id}`,transactionId:delivery.transactionId,inputs:delivery.inputs});
...
export function saveReconstructedReservation(profileId:string,id:string,transactionId:string,inputs:NonNullable<ReservedOperation['inputs']>,network:TestNetwork='signet') {
  const operations=walletReservations(profileId,network),record=operations.find(r=>r.id===id&&r.transactionId===transactionId);
