...
export function AccountActivity({ activity, onDetailChange, context }: { activity: BisActivity; onDetailChange: (open: boolean) => void; context?: Pick<BisContext, 'checkAccountTransfer' | 'closeAccount' | 'refreshActivity'> & Partial<RecoveryContext & Pick<BisContext,'checkContracts'>> }) {
  ...
  useEffect(()=>{
    ...
    const refresh=async()=>{if(reading)return;reading=true;try{const result=await context.checkContracts!({includeResolved:true});if(active){setContractError(result.status!=='ready');if(result.status==='ready')setContracts(result.contracts);}}catch{if(active)setContractError(true);}finally{reading=false;}};
  ...
  return <Page title={opened?'Transaction Detail':'Transactions'} body={opened?'Inspect this transaction and its recovery status.':'Transactions recorded for this account.'}
    ...
    headingActions={<IconButton label="Refresh Transactions" disabled={loading} onClick={()=>void context?.refreshActivity()}><span aria-hidden="true">↻</span></IconButton>}
    items={rows.map(row=>{const presentation=transactionRowPresentation(row);return {id:row.id,selected:selectedId===row.id,
      ...
      content:<><strong>{presentation.heading}</strong><span>{presentation.network}</span><code title={row.identifier}>{shortAssetId(presentation.identifier)}</code></>};})}
    ...
    notice={<>{reportError&&<p role="status">Pending recovery details unavailable. Use Refresh to retry.</p>}
      {contractError&&<p role="status">Contract activity could not be refreshed. Saved recovery remains retained.</p>}
