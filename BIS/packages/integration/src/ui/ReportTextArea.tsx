...
export function ReportTextArea({ value, ...props }: Omit<TextareaHTMLAttributes<HTMLTextAreaElement>, 'value' | 'readOnly'> & { value: string }) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // ResizeObserver delivers an initial notification after the layout effect.
      // Do not let that unchanged-size notification reset a user's first Next click.
      if (element.clientWidth === measuredWidth && element.clientHeight === measuredHeight) return;
      measuredWidth = element.clientWidth; measuredHeight = element.clientHeight;
