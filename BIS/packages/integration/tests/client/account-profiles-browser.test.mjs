...
test('encrypted player profiles migrate, deduplicate, select explicitly, and notify other contexts', {timeout:120000}, async t => {
  ...
  await page.goto(new URL('/BIS/packages/integration/src/client/state-layer-core/account-storage.ts',server.resolvedUrls.local[0]).href,{waitUntil:'domcontentloaded',timeout:10000});
  const result=await page.evaluate(async()=>{
    ...
    const {createAccountStorage}=await import('/BIS/packages/integration/src/client/state-layer-core/account-storage.ts');
