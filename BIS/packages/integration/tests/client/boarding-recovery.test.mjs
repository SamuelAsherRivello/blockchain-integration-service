...
import {pendingLogoutOperations,withBrowserMutation} from '../../src/client/state-layer-core/logout-cleanup.ts';
import test from 'node:test';
import assert from 'node:assert/strict';
import {readFileSync} from 'node:fs';
import {readBoardingRecord,writeBoardingRecord,withWalletMutation,assertNoPendingBoarding,createBoardingAttempt,recoverPreparedBoarding} from '../../src/client/state-layer-core/boarding-record.ts';
import {verifiedBoardingCommitment} from '../../src/client/state-layer-core/boarding-reconciliation.ts';
import {settlementTimeoutMs} from '../../src/client/state-layer-core/boarding-status.ts';
import {createAccountStorage} from '../../src/client/state-layer-core/account-storage.ts';
...
test('all account readers use read-only identities; sole signing adapter disables automatic settlement',()=>{
 for(const name of ['account','balance','addresses','funding','activity']) {
  const source=readFileSync(new URL(`../../src/client/wallet-layer-arkade/${name}.ts`,import.meta.url),'utf8');
 ...
 const source=readFileSync(new URL('../../src/client/wallet-layer-arkade/boarding.ts',import.meta.url),'utf8');assert.match(source,/settlementConfig:false/);
});
...
test('public confirmation serializes duplicate callers and reload resumes status without submission',async()=>{
 const {createContext}=await import('../../src/client/state-layer-core/context.ts');
