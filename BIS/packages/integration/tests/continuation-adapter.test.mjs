...
function fixture(t,{lose=false,affordable=true,current=true,storageFailure=false,withAssets=false,corruptAssets=false,missingAsset=false,wrongQuantity=false,coinValue}={}) {
 ...
 Object.defineProperty(globalThis,'localStorage',{configurable:true,value:{get length(){return memory.size;},key:i=>[...memory.keys()][i]??null,getItem:k=>memory.get(k)??null,setItem:(k,v)=>{if(storageFailure)throw Error('denied');memory.set(k,v);}}});
 ...
 t.mock.method(ReadonlyWallet,'create',async()=>{reads++;const destination=own;return {...wallet,getAddress:async()=>destination.encode(),getBoardingAddress:async()=> 'tb1fixture'};});
 ...
 const record={request:{operationId:'test',sats:1000,context:'run',recipient:recipient.encode()},profileId:'p',status:'pending'};
...
test('configured recipient needs no saved identity; exact 1000 sat payment persists before submit',async t=>{
 const f=fixture(t);const result=await f.run();assert.equal(result.status,'succeeded');assert.equal(result.mechanism,'game-wallet-payment');assert.equal(result.sats,1000);assert.equal(result.feeSats,0);assert.equal(result.recipient,recipient.encode());assert.equal(f.submits(),1);assert.equal(f.memory.size,2);assert.ok(f.memory.has('bis-signet-wallet-operations-v2:p'));assert.ok(f.disposed()>=2);assert.doesNotMatch([...f.memory.values()].join(''),/phrase|mnemonic|abandon/);
});
...
for(const phase of ['prepared','registered'])test(`B1 reconciles a ${phase} withdrawal reservation without logout`,async t=>{
 ...
 try {
  ...
  if(phase==='prepared') {
   ...
   assert.equal(readBoardingRecord('p','withdrawal').status,'not-submitted');
  } else {
