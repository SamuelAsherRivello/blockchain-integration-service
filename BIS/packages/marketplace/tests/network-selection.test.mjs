...
test('the Admin demo gets its funding and explorer links from the active network registry', async () => {
  const [app, panel] = await Promise.all([
    readFile(new URL('../../integration-demo/src/App.tsx', import.meta.url), 'utf8'),
    readFile(new URL('../../integration-demo/src/admin/AdminPanel.tsx', import.meta.url), 'utf8'),
  ]);
  ...
  assert.match(panel, /label=\{`Open \$\{networkLabel\(network\)\} Faucet\(s\)`\}/);
