...
- User-confirmed correction (2026-09-08): Transactions is the only player-facing destination for pending activity. Remove the separate Pending Operations button/panel; merge missing saved operations into history without duplicate rows, and put recovery information, Check Status, Copy Recovery Details and eligible unsent-draft discard in the selected transaction detail.

- Preserve the delivered Transactions action below Balance in Accounts Details, its three-line transaction rows, and single-click Transaction Detail with per-transaction Copy. Add Copy all transactions for every current record in order, one export line per transaction with full amount, direction, status, identifiers and available asset data. Clipboard failure exposes selectable export text. This reconciles the later row/detail presentation with A5's full-list export requirement.
