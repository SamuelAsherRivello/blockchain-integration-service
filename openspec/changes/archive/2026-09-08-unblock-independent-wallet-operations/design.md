...
The 2026-09-08 interview confirms a shared policy across BIS, with B1 as the first end-to-end verification target. B1 and other payments must use the same reservation calculation as sends, transfers and supported minting; do not implement a B1-only guard bypass.

For incomplete legacy reservations, attempt read-only reconstruction from supported transaction evidence and persist the verified complete input set before using it. While the set remains uncertain, explain that specific spending hold and keep receiving, inspection and recovery accessible. Reconstruction never signs, resubmits or treats missing history as proof of failure.

B1 is greyed out when verified unreserved funds cannot cover its price and applicable fees, with a visible accessible reason and available/reserved amounts where verified. Recompute availability when funds or reservations change; re-enable only when eligible and require an explicit click, never queue an automatic payment. An open Account dialog alone is not a blocker. Keep duplicate-submission protection. Unsupported independent minting stays greyed out with its specific reason while supported B1 payments, sends and transfers remain available.

Acceptance requires a live Signet B1 payment using disjoint inputs while an unrelated transfer remains pending: verify the configured game wallet receives 1,000 sats, match the payment identity and receipt, and preserve the original transfer's recovery record and reservations. Automated and isolated browser success alone do not satisfy this acceptance gate. If no independent funds are available, report live verification blocked rather than complete; do not automatically fund or clear the old transfer.
