...
- [ ] 2.2 Implement shared reservation/availability calculation; test disjoint funds, whole-input reservation, read-only reconstruction of unknown legacy inputs, explained holds while reconstruction remains uncertain, continued receiving/inspection/recovery access, account isolation and release of only one resolved operation.
...
- [ ] 3.1 Verify B1 first using the shared policy across BIS payments, transfers, sends and supported minting. Replace blanket payment/transfer/send guards with enforced unreserved input selection for quotes, Max and confirmation; verify actual submitted input sets exclude reservations in adapter tests for both transfer directions and sends.
...
- [ ] 4.3 Use the same public availability state in Admin: grey out B1 for insufficient verified unreserved funds or unverifiable readiness, show the specific reason and known available/reserved amounts, refresh and re-enable on eligibility changes without automatic payment, and verify Account being open alone never disables it. Grey out unsupported independent minting with its specific reason while enabling supported operations; verify the production API still enforces restrictions if the UI is bypassed and listAssets remains usable.
...
- [ ] 6.2 In the live Signet verification session, verify B1 first while an unrelated transfer remains pending: prove disjoint inputs, successful payment, receipt of 1,000 sats by the configured game wallet, and preservation of the original recovery record and reservations. Also verify disjoint-input sends/transfers and mint where supported; match actual outcomes and fresh balances to the recorded operations. Live B1 evidence is required for completion; automated and isolated browser tests alone are insufficient. If no independent funds exist, report that fact and the manual funding route without funding automatically or clearing the original operation.
