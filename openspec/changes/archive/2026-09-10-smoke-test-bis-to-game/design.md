...
The game has its own `openspec/` root and permanent C###/C###-T### conventions. This BIS-owned cross-project change is the coordination source; do not copy BIS artifacts into that root or allocate a duplicate change during this proposal. During a future authorized implementation, reconcile the game's existing `game-settings-menu` and `gameplay-pause` requirements for the new Account route; if a game-local change record is necessary, allocate IDs using its existing convention and link it to this change. The user authorized implementation in both projects on 2026-09-06; writes to the sibling game require the environment's supported filesystem escalation.
...
Use BIS `http://127.0.0.1:15174/` and game `http://127.0.0.1:15175/` consistently in Windows. These forward to server ports 5174 and 5175 respectively. These are different browser origins; the game will not automatically see the BIS demo's wallet. `127.0.0.1` is also a different hostname from `localhost`. Do not switch hostnames mid-test and mistake the empty account for data loss.
...
2. Run BIS checks/build, start BIS with `npm run dev --workspace @bis/integration-demo -- --port 5174 --strictPort`, and verify HTTP 200 plus demo HTML. Inspect A1–A6 menu navigation before packing; no automatic account creation or financial operations.
3. Pack/install the snapshot and peers in the game. Run game tests/build. Start game with `npm run dev -- --host 127.0.0.1 --port 5175 --strictPort` and verify HTTP 200 plus game HTML.
4. On Windows, keep one tunnel open: `ssh -N -o ExitOnForwardFailure=yes -L 127.0.0.1:15174:127.0.0.1:5174 -L 127.0.0.1:15175:127.0.0.1:5175 contabo-srive`. If 15174 is already forwarded, add only the 15175 forward in a second terminal rather than colliding. Share both browser URLs; distinguish host HTTP verification from the user's confirmed browser reachability.
...
The five-question interview is complete: Back to Settings, host-owned restart, fixed package, a fresh live account plus populated-data fixtures, and the user's Windows browser are confirmed. Exact browser/version is recorded at execution; it does not change scope. The proposal, delta spec and tasks are reconciled with these decisions, including restart ordering, memory invalidation, same-origin notifications and game/demo handlers.
...
## Follow-up UI planning — confirmed decisions

Only the Transactions list and Assets list may scroll. Every other BIS screen/content region must fit without scrolling, especially Set/Get Recovery Phrase and restoration word-entry forms. Shorten the seed-word warning and display it on at most one line; do not achieve this by clipping important text or hiding required controls. Question 2 confirms the sequence: shorten the warning at the current scale first, then try a temporary 80% embed only if screens still do not fit. No permanent CSS reduction is approved until the user reviews the experiment.

The game-owned ⚡ Account button goes above Music and SFX. Use game-owned CSS that visually matches the BIS button; do not import a private component or couple the button to private BIS selectors.

BIS must occupy the top game visual layer, including above virtual controls, with translucent black covering the full screen behind its UI and blocking underlying input. The current host z-index 100 is lower than the virtual-control layer 1040; inert input alone does not fix visual ordering.

Account ID and its copy icon belong only on the Accounts Details page. Remove the repeated Account ID subtitle from all other windows. Copy must use the complete public Account ID. Retain the current game-owned browser refresh on logout, explicitly confirmed by the user as acceptable for now.

A separate future proposal will address multiple game titles, universal wallet balance, possible game-specific asset filtering and a visible Game ID in Account Details. Do not implement those additions in the smoke-test change.

Question 3 is explicitly deferred to the separate multi-game proposal. Account reuse/shared login, Game ID ownership and filtering details remain open questions; do not interview or implement that scope until the user invokes its proposal workflow.

Question 4 confirms test targets rather than a universal minimum size: the user observes 743 × 1321 in Windows Chrome at 100% zoom and also requires no BIS page scrolling on Android. Measure CSS viewport, available game area and device pixel ratio during testing; the observed desktop dimensions are not proof of identical CSS dimensions or of Android layout. Do not silently adopt 360 × 640 as the approved minimum. Keep internal scrolling limited to Transactions and Assets lists on both platforms. Android keyboard-open behavior remains an interview question.


### UI feedback implementation checkpoint

The shortened warning fits display screens at the existing size; restoration still overflowed at 360 × 640 and 393 × 700 CSS pixels. The game now uses the approved temporary 80% embed experiment. No permanent global CSS size reductions have been made. Browser measurements cover the ordinary recovery screens, not arbitrary report length or a real Android software keyboard. Long transaction/manual-copy text retains its prior scrolling pending a no-scroll presentation design; do not mark the comprehensive scrolling task complete.

The Account host is attached to body (or the active fullscreen element), outside the game's clipped/container-scoped UI. It restores and reapplies inert sibling state on fullscreen changes and restores interaction on close. Its full-viewport black backdrop has 55% opacity. Initial import/hydration has no temporary message or visible placeholder. Failure/timeout retains Back.


### Approved native sizing and release

The user approved the 80% game appearance, explicitly requested native CSS values at 100%, and authorized committing, pushing and releasing BIS plus updating the game. v0.12.0 multiplies fixed CSS dimensions by 0.8, including typography, spacing, borders, shadows and control sizes; SVG display dimensions and inline confirmation sizing follow the same conversion. Relative proportions, animation timing, colors and wallet behavior remain unchanged. The game removes the 125% mount and scale(.8) transform. Account ID's browser-default input now has explicit matching dimensions. Geometry comparison across six screens and three viewports differs by at most 3.97px due to native fractional layout/rasterization. Permanent sizing approval is settled; long reports and actual Android keyboard acceptance remain separate incomplete work.
