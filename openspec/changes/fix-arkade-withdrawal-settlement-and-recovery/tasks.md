...
- [x] 4.2 Connect durable verified resolution to the shared wallet refresh path; verify balance, assets, Activity and B.P.1 update once for the correct account, unrelated reservations remain and persistence/refresh failures do not fabricate availability.
- [x] 4.3 Replace the misleading zero-funds explanation when all funds are reserved with an operation-specific pending/recovery reason; verify production B.P.1 presentation remains actionable, actual insufficient funds still reject and no conflicting payment reaches submission.
- [x] 4.4 Extend `continue-pending-swap-sequence.test.mjs` to retain the exact account-switch/pay/mint/pay/withdraw sequence, characterize the legitimate same-input pending rejection and prove successful B.P.1 after verified settlement or cancellation; verify real SDK issuance and batch-handler coverage remain distinct from transport fixtures and live acceptance.
...
- [ ] 5.2 Verify the real Admin recovery and B.P.1 flows in a stable identified browser build, covering navigation/reload and unavailable capabilities; record browser evidence and clearly label simulated transport fixtures.
...
- [ ] 5.4 On the same account after verified resolution, confirm an authorized 1,000-sat B.P.1 payment succeeds without logout; verify the configured game wallet receipt and retained assets. A passing mocked payment does not satisfy this gate.
