...
A.P.1 SHALL remain the entry-button demonstration. A.P.2 SHALL own creation and A.P.3 SHALL own restoration; both SHALL return to the actual active Account menu without manufacturing profiles. An active account SHALL hide Create Account and Restore Account and expose Accounts Details, the existing Send/Receive/Swap routes, Log Out and Back. Accounts Details SHALL open a submenu with Balance, Transactions immediately below Balance, Assets immediately below Transactions, and Back to Account. Opening Account or its submenu alone SHALL NOT request balances, history or assets. Balance SHALL use A.P.4's live available/total balances and Refresh; E.P.1 SHALL own Transactions and its detail/copy flow; account-assets SHALL own Assets and Asset Detail. Back from Balance, Transactions or Assets SHALL return to Accounts Details, while Back from Transaction Detail or Asset Detail SHALL return to its list. The Developer route SHALL be a nested view of Accounts Details. Opening onboarding from Developer SHALL remember that Developer was the immediate parent. Back from that onboarding view SHALL return to Developer with the Developer actions visible and onboarding closed. Back from Developer SHALL then return to Accounts Details. Opening onboarding from Balance or another non-Developer entry SHALL retain its existing parent route and Back destination. Existing recovery access, receiving, sending, transfer and logout behavior SHALL retain their separate capability boundaries. Log Out SHALL use A.P.5's production confirmation and SHALL NOT immediately clear the account. Back from Account SHALL restore the preceding host presentation. The demo SHALL NOT report unverified stories as complete.
...
- **AND** Log Out is enabled and opens the A.P.5 confirmation

#### Scenario: Route validation does not imply wallet functionality
- **WHEN** profile routing is tested and documented
- **THEN** logged-out creation entry, the active Account menu, the Accounts Details submenu, A.P.4 Balance, and A.P.5 logout are distinguished
- **AND** A.P.3 restores account access only before entering Account; balance loading belongs to A.P.4; E.P.1 owns all SDK-provided incoming and outgoing transaction history, Assets owns runtime asset inspection, and other unimplemented menu features remain deferred
...
#### Scenario: Return from onboarding to its immediate parent
- **WHEN** a player opens Developer from Accounts Details, opens Onboarding from Developer, and selects Back
- **THEN** the Developer view is restored with its actions visible and onboarding is closed
- **AND** selecting Back again returns to Accounts Details

#### Scenario: Balance onboarding keeps its existing parent
- **WHEN** a player opens Onboarding from the Account Details balance route and selects Back
- **THEN** Account Details is restored without opening Developer
