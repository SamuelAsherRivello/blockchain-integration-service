...
document.getElementById('run')!.onclick=async()=>{
  ...
  const c=createContext({load:async()=>({account,generation:0}),save:async()=>{throw Error('Unexpected write');},reset:async()=>{},subscribe:()=>()=>{}},undefined,async()=>account.profileId,undefined,async()=>({availableSats:0,totalSats:0,bitcoinSats:0,arkadeSats:0}),undefined,async()=>{if(fail)throw Error('private');return addresses;});
  ...
  try {
    ...
    fail=true;await c.refreshBalance();await tick();check([...addressInputs()].every(input=>input.value===''),'No stale addresses');
    fail=false;host.querySelector<HTMLButtonElement>('.bis-pending-dialog button')!.click();await tick();c.openAccountReceive();await waitFor(()=>c.getState().addresses.status==='ready');await tick();check(addressInputs()[0].value===addresses.bitcoinAddress && addressInputs()[1].value===addresses.arkadeAddress,'Retry recovery');
