...
export function AccountCard({ title, description, headingRef, headingActions, className = '', children }: {
  ...
  return <section className={`bis-card${className}`} role="dialog" aria-labelledby={titleId} aria-describedby={descriptionId}>
    ...
    <p id={descriptionId} role="status">{description}</p>
    {children}
  </section>;
}
