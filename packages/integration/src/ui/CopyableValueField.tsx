...
export function CopyableValueField({ label, value, disabled = false, className = 'bis-address-row', copy: suppliedCopy, feedback = true, selectOnFocus = true }: {
  label: string; value: string; disabled?: boolean; className?: string; copy?: ClipboardCopy; feedback?: boolean; selectOnFocus?: boolean;
}) {
  const id = useId();
  const ownCopy = useClipboardCopy(() => value, value, disabled);
  const copy = suppliedCopy ?? ownCopy;
  return <div className={className}>
    <CopyFieldLabel htmlFor={id} label={label} copied={copy.status === 'copied'} disabled={disabled || copy.status === 'copying'} onCopy={() => void copy.copy()} />
