...
import { AccountAddresses, AddressRow } from './AccountAddresses';
...
export function BisView({ context }: { context: BisContext }) {
  ...
  const menu = state.phase === 'active' && !details && !activity && !savedRecovery && !receive && !send;
  ...
  }, [state.view, state.phase, state.accountDetails, state.accountActivity, state.accountRecovery, state.accountReceive, state.accountSend]);
  if (state.view === 'empty') return null;
  const title = send ? 'Send' : receive ? 'Receive' : savedRecovery ? 'Recovery Phrase' : activity ? 'Account Activity' : details ? 'Account Details' : restoring ? 'Restore Account' : logout ? 'Account Log Out' : recovery ? 'Account Recovery' : state.phase === 'creating' ? 'Create Account' : 'Account';
    ...
    {state.view === 'account-button' ? <button ref={button} className="bis-button bis-primary" onClick={() => context.openAccountDialog()}><span aria-hidden="true">⚡</span> Account</button> :
      <section className="bis-card" role="dialog" aria-labelledby={titleId} aria-describedby={descriptionId}>
        ...
        <div className="bis-dialog-heading">
          ...
          {(details || activity || receive) && <button type="button" className="bis-copy-icon bis-title-icon" aria-label={`Refresh ${title}`} title={`Refresh ${title}`} disabled={receive ? state.addresses.status === 'idle' || state.addresses.status === 'loading' : activity ? state.activity.status === 'idle' || state.activity.status === 'loading' : state.balance.status === 'loading' || state.balance.status === 'idle'} onClick={()=>void (activity ? context.refreshActivity() : context.refreshBalance())}>
            <span className="bis-refresh-image" aria-hidden="true" />
          </button>}
        </div>
        <p id={descriptionId} role="status">{state.error ?? (send ? 'Coming soon.' : receive ? 'Use these addresses to receive test funds only.' : savedRecovery ? 'Anyone with this phrase can access your account.' : restoring ? 'Enter the recovery words saved from this experience.' : logout ? 'Did you back up your wallet?' : state.phase === 'loading' ? 'Opening your account…' : state.phase === 'resetting' ? 'Resetting your account…' : state.hasProfile ? (details || activity ? <>Account ID: <code>{state.profileId ? state.profileId.slice(0, 4) + '…' + state.profileId.slice(-4) : ''}</code></> : 'You are logged in.') : recovery ? 'Save these words privately.' : state.phase === 'creating' ? 'Creating your test account…' : 'You are not logged in.')}</p>
        {activity && <AccountActivity activity={state.activity} />}
        {details && <>
          <div className="bis-addresses">
            <AddressRow label="Available balance" address={state.balance.status === 'ready' ? `${state.balance.availableSats.toLocaleString('en-US')} sats` : state.balance.status === 'unavailable' ? 'Balance unavailable' : 'Loading...'} disabled={state.balance.status !== 'ready'} />
            <AddressRow label="Total balance" address={state.balance.status === 'ready' ? `${state.balance.totalSats.toLocaleString('en-US')} sats` : state.balance.status === 'unavailable' ? 'Balance unavailable' : 'Loading...'} disabled={state.balance.status !== 'ready'} />
          </div>
        </>}
