// Run from BIS root. Fresh browser profiles and synthetic identity only.
...
try {
 ...
 await a.evaluate(async(sourceRoot)=>{const {createAccountStorage}=await import('/@fs'+sourceRoot+'/packages/integration/src/core/account-storage.ts');const {createContext}=await import('/@fs'+sourceRoot+'/packages/integration/src/core/context.ts');const storage=createAccountStorage();await storage.save({phrase:'fixture-only',profileId:'fixture-public'},0,new AbortController().signal);window.fixtureContext=createContext(storage,async()=>{throw Error('unused')},async()=> 'fixture-public');await window.fixtureContext.ready();}, process.cwd());
 await b.goto(process.argv[2] ?? 'http://127.0.0.1:5174/');
 await b.evaluate(async(sourceRoot)=>{const {createAccountStorage}=await import('/@fs'+sourceRoot+'/packages/integration/src/core/account-storage.ts');const {createContext}=await import('/@fs'+sourceRoot+'/packages/integration/src/core/context.ts');const c=createContext(createAccountStorage(),async()=>{throw Error('unused')},async()=> 'fixture-public');await c.ready();window.fixtureContext=c;window.fixtureEvents=[];c.onEvent(e=>window.fixtureEvents.push(e.type));sessionStorage.setItem('bis.integration-demo.preview-scale','fixture');const remove=Storage.prototype.removeItem;window.fixtureFail=true;Storage.prototype.removeItem=function(key){if(window.fixtureFail&&this===sessionStorage&&key.startsWith('bis.'))throw Error('synthetic cleanup denial');return remove.call(this,key);};}, process.cwd());
